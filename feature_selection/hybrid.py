...
def recursive_feature_elimination_rf(X_train,y_train,X_test,y_test,
    ...
    for count, feature in enumerate(X_train.columns, start=1):
        print()
        ...
        print(f'New Test ROC AUC={auc_score_int}')

        # print the original roc-auc with all the features
        print(f'All features Test ROC AUC={auc_score_all}')
        ...
        if diff_auc >= tol:
            print(f'Drop in ROC AUC={diff_auc}')
            print('keep: ', feature)
...
def recursive_feature_addition_rf(X_train,y_train,X_test,y_test,
    ...
    for count, feature in enumerate(X_train.columns[1:], start=1):
        print()
        print('testing feature: ', feature, ' which is feature ', count,
          ' out of ', len(X_train.columns))
        model = RandomForestClassifier(n_estimators=n_estimators,max_depth=max_depth,
        ...
        if diff_auc >= tol:
            ...
            print(f'Increase in ROC AUC={diff_auc}')
            print('keep: ', feature)
            auc_score_all = auc_score_int
